...
def dft_1d(g, x, fx, dft_matrix = None, backend = BACKENDS["numpy"]):
    ...
    if dft_matrix is None:
        dft_matrix = backend.exp(-2j * backend.pi * backend.outer(fx, m * dx))
...
def dift_1d(G, x, fx, x_reconstruction, dift_matrix=None, backend=BACKENDS["numpy"]):
    ...
    M = x.shape[0]
    M_reconstruction = x_reconstruction.shape[0]

    x = -x
    # Get the minimum value of the spatial signal
    x_min = backend.min(x)

    # Shift the reconstruction window to the positive region
    x_reconstruction = x_reconstruction - x_min
...
def dift_2d(G, x, y, fx, fy, x_reconstruction, y_reconstruction, dift_matrix_x=None, dift_matrix_y=None, backend=BACKENDS["numpy"]):
    ...
    x = -x
    y = -y
    # Get the minimum values of the spatial signals
    x_min = backend.min(x)
    y_min = backend.min(y)

    # Shift the reconstruction windows to the positive region
    x_reconstruction = x_reconstruction - x_min
    y_reconstruction = y_reconstruction - y_min
    ...
    if dift_matrix_x is None:
        ...
        if backend == np:
            ...
            dift_matrix_x = dift_matrix_x.type(torch.complex128)
    ...
    if dift_matrix_y is None:
        ...
        if backend == np:
            ...
            dift_matrix_y = dift_matrix_y.type(torch.complex128)
...
if __name__ == "__main__":
    ...
    fx = backend.fft.fftfreq(len(x), backend.diff(x)[0])
    fx = backend.fft.fftshift(fx)
    ...
    fx_smaller = backend.arange(-15, 15, 1)
    ...
    x_reconstruction = backend.linspace(-0.5, 0.5, M)
    ...
    # Get the frequency domain - for now the whole range
    fx = backend.fft.fftfreq(len(x), backend.diff(x)[0])
    fy = backend.fft.fftfreq(len(y), backend.diff(y)[0])

    fx = backend.fft.fftshift(fx)
    fy = backend.fft.fftshift(fy)
    fxx, fyy = backend.meshgrid(fx, fy)
    ...
    im0 = ax[0].imshow(g.squeeze().numpy().real, extent = [x[0], x[-1], y[0], y[-1]])
    ax[0].set_xlabel("x")
    ax[0].set_ylabel("y")
    ax[0].set_title("Original signal")
    # Create the colorbar
    ...
    im1 = ax[1].imshow(G_backend_amp, extent = [fx[0], fx[-1], fy[0], fy[-1]])
    ...
    im2 = ax[2].imshow(G_numpy_amp, extent = [fx[0], fx[-1], fy[0], fy[-1]])
    ...
    im3 = ax[3].imshow(diff, extent = [fx[0], fx[-1], fy[0], fy[-1]])
    ...
    # Define the reconstruction window - for now the same size and discretization as the original signal
    x_reconstruction = backend.linspace(-1, 1, M)
    y_reconstruction = backend.linspace(-1, 1, N)
    ...
    im1 = ax[1].imshow(g_reconstructed_backend.squeeze().real, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
    ...
    im2 = ax[2].imshow(g_reconstructed_numpy.squeeze().real, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[2].set_xlabel("x")
    ax[2].set_ylabel("y")
    ...
    im3 = ax[3].imshow(diff.squeeze().real, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[3].set_xlabel("x")
    ax[3].set_ylabel("y")
    ax[3].set_title("Difference")
    # Create the colorbar
    ...
    x = backend.linspace(-2.5/2, 2.5/2, img.shape[-2])
    y = backend.linspace(-0.765/2, 0.765/2, img.shape[-1])
    X, Y = backend.meshgrid(x, y)
    ...
    im0 = ax[0].imshow(img.squeeze().real.T, extent = [x[0], x[-1], y[0], y[-1]])
    ax[0].set_xlabel("x")
    ax[0].set_ylabel("y")
    ...
    # Define the reconstruction window - for now the same size and discretization as the original signal
    x_reconstruction = backend.linspace(-2.5/2, 2.5/2, img.shape[-2])
    y_reconstruction = backend.linspace(-0.765/2, 0.765/2, img.shape[-1])
    ...
    im1 = ax[1].imshow(g_reconstructed_backend.squeeze().real.T, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
    ax[1].set_title("Custom 2D DIFT \nbackend = {}".format(backend_str))
    # Create the colorbar
    ...
    im2 = ax[2].imshow(g_reconstructed_numpy.squeeze().real.T, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[2].set_xlabel("x")
    ax[2].set_ylabel("y")
    ax[2].set_title("DIFT with numpy")
    # Create the colorbar
    ...
    im3 = ax[3].imshow(diff.squeeze().real.T, extent = [x_reconstruction[0], x_reconstruction[-1], y_reconstruction[0], y_reconstruction[-1]])
    ax[3].set_xlabel("x")
    ax[3].set_ylabel("y")
    ax[3].set_title("Difference")
    # Create the colorbar
    ...
    x_reconstruction_smaller = backend.linspace(1.21, 1.23, 2000)
    y_reconstruction_smaller = backend.linspace(-0.765/2, 0.765/2, 2168)
    ...
    ln0 = ax.plot(x_reconstruction_smaller, g_reconstructed_backend[:,2168//2].real, label = "Custom DIFT with backend = {}".format(backend_str))
    ln1 = ax.plot(x_reconstruction, g_reconstructed_numpy[:,2168//2].real, linestyle='dashed', label = "DIFT with numpy")
    ax.scatter(x_reconstruction, g_reconstructed_numpy[:,2168//2].real, color = "red", s = 10, label='Numpy samples (N={})'.format(num_points))
    ax.set_xlim(x_reconstruction_smaller[0], x_reconstruction_smaller[-1])
    ax.set_xlabel("x")
    ax.set_ylabel("y")
